                        ...
                        <div class="form-group text-center">
                            <div class="btn-group w-full buttons-filter" data-toggle="buttons">
                                ...
                                <button @click="isFilter = 'dominio'" :class="isFilter === 'dominio' ? 'active' : ''"
                                    :disabled="filter.TYPE === 'dominio'" class="btn btn-success p-3"
                                    style="display: flex!important; align-items: center; justify-content: center;">
                                    ...
                                    <div class="row p-1">
                                        ...
                                        <div class="col-12 p-2 container-buttons">
                                            <div style="display: flex; justify-content: start;gap:1rem">
                                                <button v-if="role?.PERM?.some((valor) => valor == '1')"
                                                    class="btn btn-view" title="Previsualizar"
                                                    @click="print(item.ENTRIEFILE)">
                                                    <i class="fas fa-eye"></i>
                                                </button>

                                                <button class="btn btn-primary" @click="download(item)"
                                                    v-if="role?.PERM?.some((valor) => valor == '2')"
                                                    title="Descargar Documento">
                                                    <i class="fas fa-download"></i>
                                                </button>

                                                <button
                                                    v-if="typeSaarch == 'jurisprudences' && item.TYPE == 'jurisprudences' && role?.PERM?.some((valor) => valor == '2')"
                                                    class="btn btn-export" @click="createPDF(item)"
                                                    title="Descargar Resumen Ejecutivo">
                                                    <i class="fas fa-file-pdf"></i>
                                                </button>

                                                <!-- v-if="role?.PERM?.some((valor) => valor == '3')" -->
                                                <div v-if="!item.IDFAV">
                                                    <button v-if="role?.PERM?.some((valor) => valor == '3')"
                                                        title="Agregar a favoritos" class="btn btn-warning text-white"
                                                        @click="favoriteAdd(item)">
